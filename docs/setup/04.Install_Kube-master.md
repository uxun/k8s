...
| scheduler          | 负责资源调度（Pod调度）的进程，相当于地铁的调度室            | 1.监听kube-apiserver，查询还未分配Node的Pod 2.根据调度策略为这些Pod分配节点 |
| controller-manager | 负责资源对象的自动化控制中心，可理解为资源对象的“大总管”     | 它通过apiserver监控整个集群的状态，并确保集群处于预期的工作状态 |
